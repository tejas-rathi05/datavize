...
import { Upload, FileText, Database, Info } from "lucide-react";
...
const CreateProjectDialog: React.FC<CreateProjectDialogProps> = ({
  ...
  const [cumulativeUploadProgress, setCumulativeUploadProgress] = useState(0);
  ...
  const onSubmit = async (data: CreateProjectFormData) => {
    try {
      ...
      if (!user || !session?.access_token) {
        throw new Error(
          "You must be logged in to create projects and upload files."
        );
      }
      ...
      if (createProject) {
        ...
        throw new Error(
          "Project creation function not provided. Please contact support."
        );
      }
      ...
      if (projectId && files.length > 0) {
        try {
          ...
          if (onFilesUploaded) {
            ...
            await uploadFilesWithProgress(projectId, files);
          }
      ...
      const message =
        files.length > 0
          ? `Project "${data.projectName}" created successfully with ${files.length} file(s) uploaded!`
    ...
    (uploadedFiles: FileList | File[]) => {
      ...
      if (storageUsed + newFilesSize > maxStorage) {
        console.log(
          ...
          formatBytes(storageUsed + newFilesSize),
          ">",
  ...
  const uploadFilesWithProgress = async (projectId: string, filesToUpload: File[]) => {
    if (!user || !session?.access_token) {
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      try {
        ...
        setCumulativeUploadProgress(progress);
        console.log(`File ${file.name} uploaded successfully (${completedFiles}/${totalFiles})`);
      } catch (error) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (input && !isProcessingFiles) {
                    console.log("Clicking file upload input from drag area");
                    input.click();
                  }
                }}
              >
                ...
                <input
                  ...
                  accept=".csv,.xlsx,.xls,.pdf,.doc,.docx,.txt,.rtf,.ppt,.pptx,.jpg,.jpeg,.png,.tiff,.zip,.eml,.msg"
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              ...
              <span>
                {fileCount} of {maxFiles.toLocaleString()} files
              </span>
            </div>
            ...
            {isSubmitting && files.length > 0 && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  ...
                  <span>{Math.round(cumulativeUploadProgress)}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    className="bg-primary h-3 rounded-full transition-all duration-300"
                    style={{ width: `${cumulativeUploadProgress}%` }}
                  />
                </div>
                <p className="text-xs text-muted-foreground">
                  {files.length} file{files.length !== 1 ? 's' : ''} being uploaded
                </p>
              </div>
            )}
